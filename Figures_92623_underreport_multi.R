...
data <- read.csv(file="Model_data_fullcountries_4323.csv")
...
DeathsAverted_byVaccinationProportion_Under60 <-ggplot(data=data_u60, aes(x=Fully_vaccinated_byAge, y= deaths_averted_incidence)) + geom_point() +
  theme_classic() + xlab("% vaccinated") + xlim(0,1) + ylab("Deaths averted, per 100,000 people") + 
  stat_poly_line() +
...
overall_plot_bytime <- ggplot(data=data5) + geom_line(aes(x=Month_Yr, y=Total_Deaths_Observed, group=Age_Cat), linetype="solid") +
  ...
  geom_ribbon(aes(x=Month_Yr, ymin=Total_Deaths_Counterfactual_low, ymax=Total_Deaths_Counterfactual_high, group=Age_Cat), alpha=0.2) +
  theme_classic() +scale_color_discrete(name="") + xlab("Date") + ylab("COVID-19 deaths")+guides(colour = "none", fill=guide_legend(title="Age")) + ggtitle("B") +
  scale_y_continuous(labels = scales::comma) + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),text=element_text(size=20)) +
  labs(caption = "Correction for 37% estimated underreporting of COVID-19 mortality (Sempé et al, 2021)") +
  facet_wrap(~ Age_Cat2) 
  
  # ggplot(data=data5) + geom_line(aes(x=Month_Yr, y=Total_Deaths_Observed, group=Age_Cat, color=Age_Cat), linetype="solid") +
  # geom_line(aes(x=Month_Yr, y=Total_Deaths_Counterfactual_average, group=Age_Cat, color=Age_Cat), linetype="dashed")+ 
  # geom_ribbon(aes(x=Month_Yr, ymin=Total_Deaths_Counterfactual_low, ymax=Total_Deaths_Counterfactual_high, group=Age_Cat, fill=Age_Cat), alpha=0.5) +
  # theme_classic() +scale_color_discrete(name="") + xlab("Date") + ylab("COVID-19 deaths")+guides(colour = "none", fill=guide_legend(title="Age")) + ggtitle("B") +
  # scale_y_continuous(labels = scales::comma) + 
  # labs(caption = "Correction for 37% estimated underreporting of COVID-19 mortality (Sempé et al, 2021)")
...
pdf(file="Output/Figure_1_92623.pdf", width=15)
overall_plot_cumulative
overall_plot_bytime
dev.off()

pdf(file="Output/Supplemental_Figure_4_10923.pdf", width=15)
